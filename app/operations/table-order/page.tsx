...
export default function TableOrderPage() {
  ...
  const calculateEstimatedTime = () => {
    ...
    const maxPreparationTime = Math.max(...order.map(item => item.preparationTime || 15))
  ...
  const serviceChargeRate = 0.10 // 10% service charge for table orders
    ...
    try {
      ...
      if (result.success) {
        ...
        setTables(tables.map(table => 
          table.id === selectedTable.id || table._id === selectedTable._id
            ? { ...table, status: 'occupied' as const, currentOrder: result.data }
            : table
        ))
        
        alert(`سفارش میز با موفقیت ثبت شد!\nمیز: ${tableOrderData.tableNumber}\nمبلغ کل: ${total.toLocaleString('fa-IR')} تومان`)
                  ...
                  >
                    <div className="relative p-4 h-full flex flex-col justify-between">
                      ...
                      <div className="flex flex-col items-center space-y-1">
                        ...
                        {table.currentOrder && (
                          <span className={`px-2 py-1 rounded-full text-xs font-bold uppercase tracking-wide transition-all duration-300 ${getOrderStatusColor(table.currentOrder.status)}`}>
